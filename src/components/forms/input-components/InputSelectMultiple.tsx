...
export default function InputSelectMultiple({ id, label, options, color, value, setValue, required, disabled, placeholder, errorMessage }: InputSelectMultipleProps) {
            ...
            >
                ...
                <div className={`flex flex-wrap gap-1 items-center ${disabled ? 'opacity-90' : ''}`}>
                    {value.map((option, index) => {
                        return (
                            <div key={`${option.value}-${index}`} className={`flex gap-2 bg-${invertedColor} text-${color} py-1 px-2 rounded-[6px]`}>
                                <p>{option.name}</p>
                                {!disabled &&
                                    <button
                                        onClick={(ev) => {
                                            ev.stopPropagation();
                                            if (disabled) return;
                                            const newValue: any = value.filter((_, i) => i !== index);
                                            setValue(newValue);
                                        }}
                                    ><svg width="8" height="7" viewBox="0 0 8 7" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1 6.32764L6.65527 0.672363M6.65527 6.32764L1 0.672363" stroke={`rgb(var(--color-${color}))`} stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round" /></svg></button>
                                }
                            </div>
                        )
                    })}
                ...
                >
                    <div className="p-1 flex flex-col gap-1 overflow-scroll max-h-[20vh]">
                        {options.filter(option => !value.find(v => v.value === option.value)).map((option, index) => {
                            return (
                                <div
                                    key={`${option.value}-${index}-add`}
                                    onClick={(ev) => {
                                        ev.stopPropagation();
                                        if (disabled) return;
